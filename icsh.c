...
"⢏⠯⢫⢫⢫⢪⢎⢯⢏⠳⡹⡹⣻⡿⡯⣫⢫⡹⡹⡽⡽⡹⡸⡜⡄⠀⠀⢀⢂⠄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡭⡭⣫⡹⡹⡭⣫⢫⢫⣚⡜⡝⡝⣝⣝⢽⡹⡭",
    ...
    for (int i = 0; art[i] != NULL; i++) {
        ...
        fflush(stdout);
        usleep(25 * 1000); // 25 ms delay
    }
}

void print_rickroll_end() {
        static const char *art[] = {
...
int handle_exit(const char *buffer) {
    ...
    print_rickroll_end(); // Print Rick Roll end art after exit prompt
    print_green("bye\n");
    return code;
}
